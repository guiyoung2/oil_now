...
| S13 | Step 6 QA 패스 — TDD(병렬 Agent A·B). a11y: MainLayout header/main 랜드마크 추가, AvgPriceCard ▲▼ aria-hidden+sr-only 패턴, PricesPage sr-only h1 추가. 색상 대비: text-gray-400 → text-gray-500/600(StationCard·PricesPage·NewsPage). pre-existing 타입 불일치 수정: AvgFuelType 분리(avgPrice.ts), PricesPage FUEL_LABELS 타입 교정. 신규 테스트 17개: a11y.test.tsx(axe 6개), keyboard.test.tsx(6개), touch-targets.test.tsx(5개). Lighthouse 스크립트(package.json) 추가. | src/components/layout/MainLayout.tsx, src/components/prices/AvgPriceCard.tsx, src/pages/PricesPage.tsx, src/pages/NewsPage.tsx, src/components/home/StationCard.tsx, src/types/avgPrice.ts, package.json, src/test/a11y.test.tsx(신규), src/test/keyboard.test.tsx(신규), src/test/touch-targets.test.tsx(신규) | 177/177 tests ✅ / npm run build ✅ | 2026-06-11 |
