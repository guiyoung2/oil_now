...
1. `CLAUDE.md` — 하네스 포인터 + 운영 규칙 확인
2. `.claude/loop/plan.md` — 존재하면 현재 Phase/Step 확인
3. `.claude/loop/fix.md` — 존재하면 미결 이슈 확인

**분기 결정:**

| 상황 | 실행 모드 |
|------|----------|
| `.claude/loop/plan.md` 없음 | 초기 실행: 템플릿으로 plan.md/fix.md 생성 + 6 Step 계획 작성 |
...
reviewer 결과를 종합하여 `.claude/loop/plan.md`와 `.claude/loop/fix.md`를 갱신한다.
